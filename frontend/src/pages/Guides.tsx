...
export const Guides = () => (
  <main className="mx-auto max-w-7xl px-4 md:px-6 py-10">
    <header className="border-b border-black pb-6 mb-10">
      ...
      <p className="text-sm text-[#525252] max-w-3xl mt-3">Original explanations connected to the formulas and evidence used across KnowYourPNL.</p>
    </header>
    <ul className="grid md:grid-cols-2 border border-black bg-white">
      {blogPosts.map((guide, index) => (
        <li key={guide.slug} className={`p-5 flex gap-3 border-[#e5e5df] ${index % 2 ? "md:border-l" : ""} ${index >= 2 ? "border-t" : ""}`}>
          <BookOpen size={16} className="mt-1 text-[#c83c2f]" />
