...
export const EvidencePanel = ({ records, title = "Evidence Sources" }: { records: EvidenceRecord[]; title?: string }) => {
  ...
  return (
    <section className="border border-[#102A43] bg-white" data-testid="evidence-panel">
      <header className="px-4 py-3 border-b border-[#102A43] flex items-center justify-between">
          ...
          <li key={r.id} className="p-4" data-testid={`evidence-row-${r.id}`}>
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="text-[12px] text-[#486581] uppercase tracking-wider mb-0.5">{r.chargeName}</div>
                <div className="text-[14px] font-medium">{r.sourceTitle}</div>
                <div className="text-[12px] text-[#486581] mt-1 break-all">
                ...
                {r.notes && <p className="text-[12px] text-[#486581] mt-2 italic">{r.notes}</p>}
              </div>
              <VerificationBadge status={r.verificationStatus} />
            </div>
            <dl className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1 text-[12px] mt-3 text-[#486581]">
              ...
              <div><dt className="kypnl-overline">Published</dt><dd className="text-[#102A43]">{r.publishedValue}</dd></div>
              {r.observedValue && <div><dt className="kypnl-overline">Observed</dt><dd className="text-[#102A43]">{r.observedValue}</dd></div>}
            </dl>
          </li>
        ))}
      </ul>
